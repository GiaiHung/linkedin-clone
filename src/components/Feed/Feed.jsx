...
function Feed() {
  ...
  return (
    <div className="feed">
      ...
      {/* Posts */}
      <FlipMove>
        {posts.map(({ name, message, url, description, id, likes, isLiked }) => (
          <Post key={id} id={id} name={name} message={message} url={url} description={description} likes={likes} isLiked={isLiked} />
        ))}
      </FlipMove>
    </div>
  )
